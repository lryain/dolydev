...
class AnimationMotorTester:
    ...
    def test_rotation(self):
        ...
        self.send_command("turn_deg",
                         {
                             "angle_deg": 90.0,     # 正值=右转
                             "throttle": 0.25,      # 转速 25%
                             "timeout_ms": 5000
                         },
                         "右转 90° (driveRotate=90, isCenter=TRUE)")
        self.wait_and_check(4.0, "等待转向完成")
        
        # 3.2 左转 90度 (负角度)
        self.send_command("turn_deg",
                         {
                             "angle_deg": -90.0,    # 负值=左转
                             "throttle": 0.25,
                             "timeout_ms": 5000
                         },
                         "左转 90° (driveRotate=-90)")
        self.wait_and_check(4.0, "等待转向完成")
        ...
        self.wait_and_check(3.0, "等待转向完成")
    ...
    def test_animation_scenario(self):
        ...
        # 前进
        self.send_command("move_distance_cm",
                         {"distance_cm": 5.0, "throttle": 0.25, "timeout_ms": 3000},
                         "前进 5cm")
        self.wait_and_check(3.0)
        
        # 右转
        self.send_command("turn_deg",
                         {"angle_deg": 45.0, "throttle": 0.25, "timeout_ms": 3000},
                         "右转 45°")
        self.wait_and_check(3.0)
