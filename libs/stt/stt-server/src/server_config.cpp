...
void ServerConfig::load_from_environment() {
    ...
    asr_config_.model_type = get_env_string("ASR_MODEL_TYPE", asr_config_.model_type);
    asr_config_.encoder_path = get_env_string("ASR_ENCODER_PATH", asr_config_.encoder_path);
    asr_config_.decoder_path = get_env_string("ASR_DECODER_PATH", asr_config_.decoder_path);
    asr_config_.joiner_path = get_env_string("ASR_JOINER_PATH", asr_config_.joiner_path);
...
void ServerConfig::load_from_args(int argc, char* argv[]) {
    ...
    for (int i = 1; i < argc; i++) {
        ...
        else if (arg == "--asr-model-type" && i + 1 < argc) {
            asr_config_.model_type = argv[++i];
        }
        else if (arg == "--encoder" && i + 1 < argc) {
            asr_config_.encoder_path = argv[++i];
        }
        else if (arg == "--decoder" && i + 1 < argc) {
            asr_config_.decoder_path = argv[++i];
        }
        else if (arg == "--joiner" && i + 1 < argc) {
            asr_config_.joiner_path = argv[++i];
        }
        // VAD配置
