...
public:
  struct ASRConfig {
    ...
    // 模型类型: "auto" 自动检测, "sense-voice" 离线, "zipformer" 流式
    std::string model_type = "auto";
    
    // Zipformer 模型文件路径（自动检测或手动指定）
    std::string encoder_path;  // encoder-epoch-99-avg-1.int8.onnx
    std::string decoder_path;  // decoder-epoch-99-avg-1.onnx
    std::string joiner_path;   // joiner-epoch-99-avg-1.int8.onnx
  };
