...
void ASRSession::request_finalize() {
    if (!running) return;
    send_final_result_from_full_audio();
}
...
void ASRSession::add_audio_data(const std::vector<uint8_t>& pcm_bytes) {
    ...
    
    {
        ...
        full_utterance_buffer.insert(full_utterance_buffer.end(), samples.begin(), samples.end());
    ...
    while (running) {
        ...
        try {
            ...
            if (!vad->IsEmpty()) {
                ...
                // 修复：处理完一个语音段后，重置偏移量并将缓冲区切片，
                // 这样后续的音频流才能从头开始进行 VAD 检测
                if (buffer.size() > static_cast<size_t>(offset.load())) {
                    buffer = std::vector<float>(buffer.begin() + offset.load(), buffer.end());
                } else {
                    buffer.clear();
                }
                offset = 0;
                ...
                // 给 VAD 发送重置状态信号，以便它可以处理接下来的音频片段
                vad->Reset();
            }
...
void ASRSession::send_final_result_from_full_audio() {
    SharedASREngine* shared_asr = engine->get_shared_asr();
    if (!shared_asr || !shared_asr->is_initialized()) {
        LOG_ERROR(client_id, "Shared ASR engine not available for finalization");
        return;
    }
    ...
    {
        std::lock_guard<std::mutex> lock(audio_mutex);
        full_audio = full_utterance_buffer;
    }

    if (full_audio.empty()) {
        LOG_WARN(client_id, "Finalize requested but no audio available");
        return;
    }
    ...
    try {
        ...
        std::string text = shared_asr->recognize_with_metadata(
            full_audio.data(), full_audio.size(),
        ...
        {
            std::lock_guard<std::mutex> lock(audio_mutex);
            std::queue<std::vector<float>> empty_queue;
            std::swap(audio_queue, empty_queue);
            full_utterance_buffer.clear();
        }
        ...
        LOG_ERROR(client_id, "Error finalizing result: " << e.what());
    }
}
