...
// 支持两种模型: SenseVoice (OfflineRecognizer) 和 Streaming Zipformer (OnlineRecognizer)
class SharedASREngine {
public:
  enum class ModelType {
    SENSE_VOICE,   // 离线SenseVoice模型
    ZIPFORMER,     // 流式Zipformer模型
    UNKNOWN        // 未知类型
  };
...
private:
  ...
  // 模型类型检测函数
  static ModelType detect_model_type(const std::string &model_dir);
  
  // 初始化 SenseVoice 离线识别器
  bool initialize_sense_voice(const std::string &model_dir,
                               const ServerConfig &config);
  
  // 初始化 Zipformer 在线识别器
  bool initialize_zipformer(const std::string &model_dir,
                             const ServerConfig &config);
...
public:
  ...
  ModelType get_model_type() const { return model_type; }

  // 线程安全的识别接口 - 统一 API
  std::string recognize(const float *samples, size_t sample_count);
  
  // 仅 SenseVoice 模型支持的扩展元数据识别
