            ...
            if (msg->get_opcode() == websocketpp::frame::opcode::binary) {
                ...
                if (reader.parse(payload, root)) {
                    if (root.get("done", false).asBool()) {
                        it->second->request_finalize();
                        LOG_INFO(client_id, "Received done=true, finalized streaming recognition");
                    } else {
                        LOG_DEBUG(client_id, "Received streaming text message without done flag");
                    }
                } else {
                    LOG_WARN(client_id, "Received non-binary non-JSON message for streaming session, ignoring");
                }
            }
            return;
        }
